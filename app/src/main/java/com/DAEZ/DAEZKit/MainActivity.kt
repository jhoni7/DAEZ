...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var btnManageModels: Button
    ...
    private fun initViews() {
        ...
        btnManageModels = findViewById(R.id.btn_manage_models)
    }
    ...
    private fun setupLanguageSpinners() {
        val languages = arrayOf(
            ...
            "Italiano", "Português", "中文 -JP", "日本語 - CH", "한국어 - KO", "Русский - RU", "العربية"
        )
    ...
    private fun showManageModelsDialog() {
        val tempManager = com.DAEZ.DAEZKit.managers.TranslatorManager()
        
        tempManager.getDownloadedModels { models ->
            if (models.isEmpty()) {
                runOnUiThread {
                    androidx.appcompat.app.AlertDialog.Builder(this)
                        .setTitle("Modelos Descargados")
                        .setMessage("No hay modelos descargados localmente.")
            ...
            val languageNames = models.map { getLanguageName(it) }.toTypedArray()
            
            runOnUiThread {
                androidx.appcompat.app.AlertDialog.Builder(this)
                    .setTitle("Modelos Descargados")
                    .setItems(languageNames) { _, which ->
                        val langCode = models[which]
                        showModelOptions(langCode, languageNames[which], tempManager)
                    }
                    .setPositiveButton("Cerrar", null)
                    .show()
            }
    ...
    private fun showModelOptions(langCode: String, langName: String, manager: com.DAEZ.DAEZKit.managers.TranslatorManager) {
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle(langName)
            .setMessage("¿Qué deseas hacer con el modelo de $langName ($langCode)?")
            .setNegativeButton("Eliminar") { _, _ ->
                manager.deleteModel(langCode) { success ->
                    runOnUiThread {
                        if (success) {
                            Toast.makeText(this, "Modelo eliminado", Toast.LENGTH_SHORT).show()
                            showManageModelsDialog() // Recargar lista
                        } else {
